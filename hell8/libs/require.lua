...
function require.tree(requirePath)
  if not cache[requirePath] then
    ...
    local entries = lfs.getDirectoryItems(fsPath)--lfs.enumerate(fsPath)

    for _,entry in ipairs(entries) do
      fsPath = toFSPath(requirePath .. '.' .. entry)
      local fsPathInfo = love.filesystem.getInfo(fsPath)
      if --[[lfs.isDirectory(fsPath)--]] fsPathInfo then
